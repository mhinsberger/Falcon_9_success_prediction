...
app.layout = html.Div(
    children=[
        ...
        # TASK 2: Add a pie chart to show
        ...
        # TASK 4: Add a scatter chart to show 
        # the correlation between payload and launch success
        html.Div(
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(
            ...
            values='class', 
            names='Launch Site', 
            title='Total success launches for By Site'
        )
        return fig
    else:
        ...
        starts = filtered_df.shape[0]
        successes = filtered_df['class'].sum()
        fails = starts - successes
        values = [fails, successes]
        names = [0, 1] 
        fig = px.pie(
            values=values,
            names=names,
...
def get_scatter_plot(entered_site, slider_values):
    filtered_df = spacex_df.loc[
        (spacex_df['Payload Mass (kg)'] >= slider_values[0]) &
